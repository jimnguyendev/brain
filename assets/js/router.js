...
export const attachSPARouting = (init, rerender) => {
  ...
  // Re-execute inline content scripts after an SPA swap. Cloned/morphed <script>
  // nodes never run on their own, so embedded note widgets (e.g. canvas/SVG demos)
  // would be inert after client-side navigation. Re-running is safe because such
  // widgets guard their own init (e.g. dataset flags).
  const reexecContentScripts = () => {
    const container = document.querySelector(".singlePage")
    if (!container) return
    container.querySelectorAll("script").forEach((old) => {
      if (old.dataset.spaRan) return
  ...
  window.addEventListener("million:navigate", () => {
    render()
    requestAnimationFrame(reexecContentScripts)
  })
}
