...
prepare-local: ## rebuild indices locally (relative paths; works on this machine, unlike `prepare`)
	hugo-obsidian -input=content -output=assets/indices -index=true -root=.
	python3 utils/lower_case.py
	python3 utils/strip_private.py
	# NOTE: local serve does NOT encrypt private pages (StatiCrypt runs only in CI deploy);
	# private notes render in full locally so you can preview them.

serve-local: prepare-local run ## prepare indices locally then serve
